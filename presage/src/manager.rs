...
{
    pub fn new(config_store: C) -> Result<Self, Error> {
    ...
    fn new_message_sender(&self) -> Result<MessageSender<C, rand::rngs::ThreadRng>, Error> {
    ...
    fn new_service_cipher(&self) -> Result<ServiceCipher<C, rand::rngs::ThreadRng>, Error> {
        let signal_servers = match &self.state {
            State::Registered { signal_servers, .. } => signal_servers,
            _ => return Err(Error::NotYetRegisteredError),
        };
