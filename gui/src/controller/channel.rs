        ...
        W: Widget<SignalData>,
{
    fn event(
        ...
        match event {
            Event::Command(cmd) if cmd.is(command::SET_CURRENT_CHANNEL) => {
                let channel = cmd.get_unchecked(command::SET_CURRENT_CHANNEL);

                data.current_channel.replace(channel.to_owned());
